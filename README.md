...
1. `() → Unit` 화살표 표기법을 사용한다.
2. 함수는 값이 될 수 있고, 역으로 값은 함수가 될 수 있다.
    1. 그러므로 함수에 인자로 넘기거나 데이터 구조에 저장할 수 있다.
3. fun으로 선언된 함수는 값으로 다룰 수 없다.
4. 함수형 프로그래밍에선 이름 없는 무명 함수를 `익명함수`라고 한다.
    1. 익명함수를 람다 표현식으로 바꿀 수 있다.
5. 함수의 마지막 인자가 함수인 경우 `후행 람다 전달`을 사용할 수 있다.
6. 람다 함수의 파라미터가 1개인 경우에만 `it`을 사용할 수 있다.
7. 람다 레퍼런스를 사용하면 좀 더 가독성 좋게 함수를 인자로 넘길 수 있다.
8. 탑-레벨 또는 로컬 함수는 `::함수명`을 사용하고 클래스의 멤버이거나 확장 함수는 클래스 지시자 `클래스명::함수명`를 사용한다.
9. 유용한 함수형 라이브러리 애로우
